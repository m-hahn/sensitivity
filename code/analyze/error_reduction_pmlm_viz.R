...
byTaskLong$Model = factor(byTaskLong$Model, levels=c("BoE", "BiLSTM", "RoBERTa"))
...
ggsave(plot, file="s1ensitivity-accuracy-grid-pmlm.pdf", width=9, height=3.6)


plot = ggplot(data=byTaskLong, aes(x=MeanS1ensitivity, y=100*ErrorReduction, color=Type)) + geom_point() + geom_label_repel(aes(label=Task), label.size=NA)
plot = plot + theme_bw()  + theme(legend.position="bottom") + facet_grid(~Model) + theme(axis.text = element_text(size=12), axis.title = element_text(size=15), strip.text = element_text(size = 18))
plot = plot + ylab("Error Reduction (%)") + xlab("Average Block Sensitivity") + ylim(0,100) + theme(legend.title = element_blank()) 


library(cowplot)
legend <- get_legend(plot)
plot = plot_grid(NULL, legend, ncol=1)
ggsave(plot, file="s1ensitivity-accuracy-grid-pmlm-legend.pdf", width=4, height=0.2)
