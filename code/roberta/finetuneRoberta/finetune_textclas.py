...
assert False, "is this the newer version, or is generate8.py?"
...
# ~/python-py37-mhahn generate8.py --model_type="XLNet" --model_name_or_path=xlnet-large-cased
...
if args.dataset == 'mr':
    ...
    data, label = dataloader.read_SUBJ(args.path, clean=False)
elif args.dataset == 'cr':
    data, label = dataloader.read_CR(args.path)
elif args.dataset == 'mpqa':
    data, label = dataloader.read_MPQA(args.path, clean=False)
elif args.dataset == 'trec':
    train_x, train_y, test_x, test_y = dataloader.read_TREC(args.path, clean=False)
    data = train_x + test_x
    label = None
elif args.dataset == 'sst':
    train_x, train_y, valid_x, valid_y, test_x, test_y = dataloader.read_SST(args.path, clean=False)
...
with open(f"/u/scr/mhahn/PRETRAINED/textclas/{args.dataset}_alternatives.txt", "w") as outFile:
