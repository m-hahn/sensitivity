...
# ~/python-py37-mhahn finetune_xlnet_textclas_8_d.py --model_type="XLNet" --model_name_or_path=xlnet-large-cased --dataset=cr/mr/subj/trec/mpqa
...
with open(f"/u/scr/mhahn/PRETRAINED/textclas/{args.dataset}_alternatives_finetuned.txt", "r") as inFile:
...
runOnCorpus(sentences[:2000], doTraining = True)
devLosses.append(runOnCorpus(sentences_dev, doTraining = False))
...
sentences = data_dev[::]
with open(f"/u/scr/mhahn/PRETRAINED/textclas/{args.dataset}_alternatives_finetuned_d.txt", "w") as outFile:
