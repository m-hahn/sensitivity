...
# Construct neighbors for Dependency Parsing

# ~/python-py37-mhahn generate7_c_ChangeAll.py --model_type="XLNet" --model_name_or_path=xlnet-large-cased
...
sentences = []
with open("/u/scr/corpora/Universal_Dependencies/Universal_Dependencies_2.4/ud-treebanks-v2.4/UD_English-EWT/en_ewt-ud-dev.conllu", "r") as inFile:
   for line in inFile:
     if line.startswith("# text ="):
        sentences.append(line[8:].strip())
######################################################################
...
with open("/u/scr/mhahn/PRETRAINED/parsing/en_ewt-ud-dev_alternatives_c_ChangeAll.txt", "w") as outFile:
...
#    # Make sure that, if the critical word is affected, it is kept entirely
#    print(tokenizedStrings[max(0,criticalWord-5):criticalWord+5])
#    firstBPEOfCurrentWord = 0
#    for i in range(criticalWord+1):
#       if tokenizedStrings[i].startswith("▁"):
#          firstBPEOfCurrentWord = i
#    firstBPEAfterCurrentWord = len(tokenizedStrings)
#    for i in range(criticalWord+1, len(tokenizedStrings)):
#       if tokenizedStrings[i].startswith("▁"):
#          firstBPEAfterCurrentWord = i
#          break
#
#    
#    subsets_ = set()
#    for subset in subsets:
#        left = subset[:firstBPEOfCurrentWord]
#        right = subset[firstBPEAfterCurrentWord:]
# #       print("BEFORE", subset)
#        subset = left + ("0"*(len(subset)-len(left)-len(right))) + right
##        print("AFTER ", subset)
...
#
#    # Make sure the critical word stays unperturbed
#    subsets_ = set()
#    for subset in subsets:
#        subset = subset[:criticalWord] + "0" + subset[criticalWord+1:]
#        assert len(subset) == sentLength, (len(subset), criticalWord, sentLength)
#        subsets_.add(subset)
#    subsets = subsets_
#  #    print(subset__)
