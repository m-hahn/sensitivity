...
# Construct neighbors for QNLI

# ~/python-py37-mhahn finetune_xlnet_qnli_22_c.py --model_type="XLNet" --model_name_or_path=xlnet-large-cased --dataset=QNLI
...
assert args.dataset == "QNLI"

import random

sentences = []
with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/{args.dataset}/train.tsv", "r") as inFile:
...
with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/{args.dataset}/dev.tsv", "r") as inFile:
  ...
  for line in inFile:
     ...
     if len(line) < 3:
       continue
     #assert len(line) == 4
     sentences_dev.append((line[1], line[2]))
...
scheduler = get_linear_schedule_with_warmup(
    optimizer, num_warmup_steps=10, num_training_steps=1000
)
...
def runOnCorpus(sentences, doTraining = False):
 ...
 losses = []
 for prompt_num, datapoint in enumerate(sentences):
    prompt_text1 , prompt_text2 = datapoint
...
#    print("NUMBER OF SUBSETS", 2**sentLength)
    encoded = torch.LongTensor(encoded).view(1, -1).cuda()
    input_ids = encoded
...
#       quit()      
      ...
      # print(ceLoss)
       ...
       print(devLosses)
       print(float(loss), prompt_num/(len(sentences)), sum(losses)/len(losses), doTraining)
       if doTraining:
          model.zero_grad()
...
devLosses.append(runOnCorpus(sentences_dev[:1000], doTraining = False))
runOnCorpus(sentences[:1000], doTraining = True)
devLosses.append(runOnCorpus(sentences_dev[:1000], doTraining = False))
...
with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/{args.dataset}/dev_alternatives_c_finetuned.tsv", "w") as outFile:
                 ...
                 if i+2 < len(subset) and subset[i+1] == "0":
                    if not  tokenizedStrings[i+1].startswith("▁"):
                        for j in range(lastStart, i+1):
                           subset[j] = "0"
        subset[-1] = "0" # Not sure if the preceding loop might have set this to 1, making it 0 just to be sure
