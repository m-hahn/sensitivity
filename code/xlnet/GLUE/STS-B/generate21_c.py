...
# Construct neighbors for STS-B

# ~/python-py37-mhahn generate21_c.py --model_type="XLNet" --model_name_or_path=xlnet-large-cased --dataset=STS-B
...
with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/{args.dataset}/dev.tsv", "r") as inFile:
  ...
  for line in inFile:
     ...
     if len(line) < 5:
       continue
     #assert len(line) == 4
     sentences.append((line[7], line[8]))
