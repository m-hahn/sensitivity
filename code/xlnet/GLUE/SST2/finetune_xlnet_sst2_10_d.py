...
# ~/python-py37-mhahn finetune_xlnet_sst2_10_d.py --model_type="XLNet" --model_name_or_path=xlnet-large-cased --dataset=SST2
...
with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/SST-2/dev_alternatives_d_finetuned.tsv", "w") as outFile:
  for prompt_text in sentences[24:]:
    prompt_text = prompt_text + " </s>"
    # Different models need different input formatting and/or extra arguments
    requires_preprocessing = args.model_type in PREPROCESSING_FUNCTIONS.keys()
    if requires_preprocessing:
