...
# ~/python-py37-mhahn finetune_xlnet_sst2_10_c_sentBreak_new.py --model_type="XLNet" --model_name_or_path=xlnet-large-cased --dataset=SST2
...
LEARNING_RATE = random.choice([5e-6, 1e-5, 5e-5])
WARMUP_STEPS = random.choice([10, 100, 1000])
TRAINING_STEPS = random.choice([100000, 1000000, 10000000])
...
for _ in range(10):
   runOnCorpus(sentences[:20000], doTraining = True)
   devLosses.append(runOnCorpus(sentences_dev, doTraining = False))
...
with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/SST-2/dev_alternatives_c_sentBreak_new_large_finetuned.tsv", "w") as outFile:
