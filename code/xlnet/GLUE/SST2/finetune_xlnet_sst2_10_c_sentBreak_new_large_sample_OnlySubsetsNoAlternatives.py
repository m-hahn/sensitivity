...
with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/SST-2/dev_alternatives_c_sentBreak_new_finetuned_large_L811_OnlySubsetsNoAlternatives.tsv", "w") as outFile:
  for prompt_text in sentences:
    if len(prompt_text.strip().split(" ")) not in [8, 9, 10, 11]:
       continue
...
#    quit()
    ...
    if sentLength <= 14:
      spanLength = 1
       ...
       spanLength = int(sentLength/14)+1
    # spans of length sentLength/11
    for subset in range(1, 2**14-1):
      subset_ = format(subset, "b")
      subset_ = ("0"*(14 - len(subset_))) + subset_
      assert len(subset_) == 14, subset_
   #   print(subset_)
