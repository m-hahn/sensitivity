...
# ~/python-py37-mhahn finetune_xlnet_sst2_10_d_sentBreak.py --model_type="XLNet" --model_name_or_path=xlnet-large-cased --dataset=SST2
...
runOnCorpus(sentences[:1500], doTraining = True)
devLosses.append(runOnCorpus(sentences_dev, doTraining = False))
assert devLosses[1] < devLosses[0]
runOnCorpus(sentences[1500:3000], doTraining = True)
devLosses.append(runOnCorpus(sentences_dev, doTraining = False))
assert devLosses[2] < devLosses[1]
#runOnCorpus(sentences[3000:4500], doTraining = True)
#devLosses.append(runOnCorpus(sentences_dev, doTraining = False))
#assert devLosses[3] < devLosses[2]
...
with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/SST-2/dev_alternatives_d_sentBreak_finetuned.tsv", "w") as outFile:
  for prompt_text in sentences[25:]:
