...
# ~/python-py37-mhahn finetune_xlnet_sst2_10_c_sentBreak_new_large_sample_temp_d.py --model_type="XLNet" --model_name_or_path=xlnet-large-cased --dataset=SST2
...
with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/SST-2/dev_alternatives_c_sentBreak_new_finetuned_large_temp_{TEMPERATURE}_d.tsv", "w") as outFile:
  for prompt_text in sentences[200:]:
    ...
    #    print(len(points))
        if len(points) == BATCH_SIZE or len(queue) == 0:
            ...
            with torch.no_grad():
              ...
              next_token_logits = TEMPERATURE * outputs[0]  # Output has shape [target_mapping.size(0), target_mapping.size(1), config.vocab_size]
