...
# Construct neighbors for CoLA

# ~/python-py37-mhahn finetune_xlnet_cola_1.py --model_type="XLNet" --model_name_or_path=xlnet-large-cased --dataset=CoLA
...
assert args.dataset == "CoLA"

import random

sentences = []
with open("/u/scr/mhahn/PRETRAINED/GLUE/glue_data/CoLA/train.tsv", "r") as inFile:
  for line in inFile:
...
sentences_dev = []
with open("/u/scr/mhahn/PRETRAINED/GLUE/glue_data/CoLA/dev.tsv", "r") as inFile:
  for line in inFile:
     line = line.strip().split("\t")
     if len(line) < 2:
       continue
     assert len(line) == 4
     sentences_dev.append(line[3])
...
#       quit()      
      ...
      # print(ceLoss)
       ...
       print(devLosses)
       print(float(loss), prompt_num/len(sentences), sum(losses)/len(losses))
       if doTraining:
          model.zero_grad()
...
with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/CoLA/dev_alternatives_c_finetuned.tsv", "w") as outFile:
