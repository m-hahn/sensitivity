...
# Construct neighbors for QQP

# ~/python-py37-mhahn finetune_xlnet_qqp_11_c.py --model_type="XLNet" --model_name_or_path=xlnet-large-cased --dataset=QQP
...
with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/{args.dataset}/dev.tsv", "r") as inFile:
  ...
  for line in inFile:
     ...
     if len(line) < 6:
       continue
     assert len(line) == 6
     sentences_dev.append((line[3], line[4]))
...
def runOnCorpus(sentences, doTraining = False):
 ...
 for prompt_num, datapoint in enumerate(sentences):
    if prompt_num > 2000:
      break
    datapoint1 , datapoint2 = datapoint
    prompt_text1 = datapoint1
    prompt_text2 = datapoint2

    prompt_text = prompt_text1.strip() + " " + prompt_text2.strip() + " </s>"
...
#       quit()      
      ...
      # print(ceLoss)
       ...
       print(devLosses)
       print(float(loss), prompt_num/min(2000, len(sentences)), sum(losses)/len(losses), doTraining)
       if doTraining:
          model.zero_grad()
...
runOnCorpus(sentences, doTraining = True)
devLosses.append(runOnCorpus(sentences_dev, doTraining = False))
...
with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/{args.dataset}/dev_alternatives_c_finetuned.tsv", "w") as outFile:
  for prompt_text1, prompt_text2 in sentences:
    ...
    print(tokenized_bpe)
    questionMarks = [i for i in range(len(tokenized_bpe)) if "?" in tokenized_bpe[i]]
    assert len(questionMarks) >= 2
#    assert False
    ...
    print(" ".join([str(x) for x in questionMarks]), file=outFile) # record the boundaries between the two questions
    sentLength = len(encoded) - LENGTH_OF_INITIAL_PART
