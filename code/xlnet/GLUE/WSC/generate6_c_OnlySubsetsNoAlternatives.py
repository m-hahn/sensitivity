...
# Construct neighbors for WSC

# ~/python-py37-mhahn generate6_c_OnlySubsetsNoAlternatives.py --model_type="XLNet" --model_name_or_path=xlnet-large-cased
...
#####################################################################
# Collect sentences to construct alternatives for (specific to WSC)
with open("/u/scr/mhahn/PRETRAINED/WSC/val.txt", "r") as inFile:
...
with open("/u/scr/mhahn/PRETRAINED/WSC/val_alternatives_c_OnlySubsetsNoAlternatives.txt", "w") as outFile:
  for prompt_text in sentences:
    # Take care of potentially absent punctuation
    if len(prompt_text) > 0 and prompt_text[-1] not in ["."]:
      prompt_text += "."
    print("SENTENCES", prompt_text, "#")
    if len(prompt_text) < 3: # Empty sentence
       continue

    # Take care of punctuation indicating pronoun and named entity (specific to WSC)
    prompt_text = " "+prompt_text
    prompt_text = prompt_text.replace("[", "[ ")
#    prompt_text = prompt_text.replace("]", " ]")
    prompt_text = prompt_text.replace("_", " _ ")
    prompt_text = prompt_text.replace("  ", " ")
#    prompt_text = prompt_text.replace(" .", ".")
    ...
    if requires_preprocessing:
        ...
        assert False, "I think this shouldnt happen, based on MNLI code"
        encoded_prompt = tokenizer.encode(prompt_text, add_special_tokens=False, return_tensors="pt")
    ...
    print([tokenizer.convert_ids_to_tokens(int(x)) for x in encoded_prompt[0]])
#    quit()
    ...
    ######################################3
    # Take care of pronoun and named entity
    start_underscore = None
    end_underscore = None
    start_bracket = None
    end_bracket = None
    ...
    while i <  len(encoded):
      token = tokenizer.convert_ids_to_tokens(int(encoded[i]))
      if (token == '▁_' or token == '_') and start_underscore is not None:
        end_underscore = i-LENGTH_OF_INITIAL_PART
        del encoded[i]
        i-=1
      elif (token == '▁_' or token == '_') and start_underscore is None:
        start_underscore = i-LENGTH_OF_INITIAL_PART
        del encoded[i]
        i-=1
      elif (token == '▁[' or token == '[') and start_bracket is None:
        start_bracket = i-LENGTH_OF_INITIAL_PART
        del encoded[i]
        i-=1
      elif (token == '▁]' or token == ']') and start_bracket is not None:
        end_bracket = i-LENGTH_OF_INITIAL_PART
        del encoded[i]
        i-=1
      print(i, token, start_underscore, end_underscore, start_bracket, end_bracket, (token == '_'))
      i += 1
    assert start_underscore is not None
    assert end_underscore is not None
    assert start_bracket is not None
    assert end_bracket is not None
    assert start_underscore < end_underscore
    assert start_bracket < end_bracket
    ###################################################
    print("#####", file=outFile)
    print(prompt_text, file=outFile)
    print(start_underscore, end_underscore, start_bracket, end_bracket, file=outFile)    
...
#    print(subsets)
    ...
    for subset in subsets:
        subset = list(subset)
        # Make sure the named entity and the pronoun remain intact
        for i in range(start_underscore, end_underscore):
          subset[i] = "0"
        for i in range(start_bracket, end_bracket):
          subset[i] = "0"
        subset[-1] = "0"
        if "1" not in subset:
          continue
        subset = "".join(subset)
        # Sanity checking
        assert len(subset) == sentLength, (len(subset), criticalWord, sentLength)
        subsets_.add(subset)
    subsets = subsets_
