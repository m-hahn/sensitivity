...
# Construct neighbors for SyntaxGym_248

# Used to be called generate23_c.py
# ~/python-py37-mhahn generate23_c.py --model_type="XLNet" --model_name_or_path=xlnet-large-cased --dataset=SyntaxGym_248
...
assert args.dataset == "SyntaxGym_248"

import json
sentences = []
with open(f"/u/scr/mhahn/PRETRAINED/SyntaxGym/syntaxgym_{args.dataset.split('_')[1]}.json", "r") as inFile:
  dataset = json.loads(inFile.read().strip())
  for item in dataset["items"]:
    for condition in item["conditions"]:
      ...
      for region in condition["regions"]:
         if region["region_number"] == 7:
            sentence+="@ "
         sentence += region["content"] + " "
#         indicesForRegions.append(len(sentence))     
 #     intro, np_subject, prep, the, prep_np, matrix_v, reflexive = indicesForRegions
...
with open(f"/u/scr/mhahn/PRETRAINED/SyntaxGym/syntaxgym_{args.dataset.split('_')[1]}_alternatives.tsv", "w") as outFile:
...
#    quit()
    ...
    if tokenized_bpe[separationMarks[0]-1] == "▁":
       del encoded[separationMarks[0]-1+LENGTH_OF_INITIAL_PART]

    print([tokenizer.convert_ids_to_tokens(x) for x in encoded[LENGTH_OF_INITIAL_PART:]])
    if not tokenizer.convert_ids_to_tokens(encoded[separationMarks[0]+LENGTH_OF_INITIAL_PART-1]).endswith("?"):
       print("NOT ENDING WITH QUESTION MARK???")
    #quit()
...
#    quit()
    ...
    spanLength = int((sentLength-3)/7)+1
   ...
   #   print(subset_)
      subset__ = "".join([x*spanLength for x in subset_]) + "000"
...
#    quit()
    ...
    for subset in subsets:
        subset = list(subset)
        subset[-1] = "0"
        for i in range(separationMarks[0], len(subset)):
          subset[i-1] = "0"
#          assert i > 0
 #         assert i < len(subset)
        if "1" not in subset:
