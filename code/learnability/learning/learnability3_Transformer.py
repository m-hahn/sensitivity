...
def fitNetwork(function):
   ...
   positional_embeddings = torch.nn.Embedding(10, hidden_size//2).cuda()
   qrnn = torch.nn.TransformerEncoder(encoder_layer = torch.nn.TransformerEncoderLayer(d_model=hidden_size, nhead=4, dim_feedforward=32, dropout=0.1, activation='relu'), num_layers=4).cuda()
...
with open(f"output/losses_{__file__}.tsv", "w") as outFile:
  print("\t".join(["AverageDegree", "Acc100", "Acc1000", "Acc10000", "Acc100000"]), file=outFile)
  for _ in range(5):
   for j in range(0, N+2):
    print("Sens", j)
    function, averageDegree = sampleFunction(j) 
    loss = fitNetwork(function)[:4]
    print(loss, averageDegree, loss)
    print("\t".join([str(x) for x in ([float(averageDegree)] + loss)]), file=outFile)
