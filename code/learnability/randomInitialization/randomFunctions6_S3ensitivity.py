...
def computeAverageBlockS3ensitivity(function):
    totalAverageBlockS3ensitivity = 0
...
#        sumSquaredValue = 0
 ...
 #            sumSquaredValue += (value ** 2) * probHere
        ...
        if totalProbability == 0: 
           ...
           expectation = sumValue/totalProbability
           squaredDistance = pow(float(expectation - function[input_string]), 2)
    #    print(variance)
    #    print(input_string, subset, probs, prob,entropy)
        perSubsetSensitivities[subset] = squaredDistance
...
#     print("per subset sensitivities", perSubsetSensitivities)
     ...
     best = bestFractional
    #  print(best)
     totalAverageBlockS3ensitivity +=  weights[input_string] * best
    return totalAverageBlockS3ensitivity
...
with open("output/randomFunct_S3ensitivity.tsv", "w") as outFile:
 print("AverageBlockS3ensitivity", file=outFile)
 for trial in range(100):
    print(trial)
    function = torch.FloatTensor([1 if random.random() > 0.5 else -1 for _ in range(2**N)])
    print(float(computeAverageBlockS3ensitivity(function)), file=outFile)
