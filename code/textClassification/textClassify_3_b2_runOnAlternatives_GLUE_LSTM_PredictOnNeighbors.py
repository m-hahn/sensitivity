...
if args.dataset in ["CoLA", "SST-2"]:
  ...
  data_train = [clean_str(x+" # "+y).split(" ") for x, y in zip(data_train0, data_train1)]
  data_dev = [clean_str(x+" # "+y).split(" ") for x, y in zip(data_dev0, data_dev1)]
...
class Model(nn.Module):
   def __init__(self):
       ...
       self.rnn = nn.LSTM(300, 128, bidirectional=True)
 ...
 #      self.rnn2 = nn.LSTM(300, 128, 1, bidirectional=True)
   ...
   def forward(self, current, printHere=False, computeAccuracy=False, train=True, getPredictions=False, outFile=None):
       ...
       if outFile is not None:
          premises = [clean_str(x.replace("<s>", "").replace("</s>", "").strip().replace("@", "#")).split(" ") for x in current]
  #        print(premises)
   #       quit()
       else:
...
#       print(list(zip(premises, targets)))
 #      quit()
       ...
       _, hidden = self.rnn(words_layer_premise.transpose(0,1))
       hidden = hidden[0].transpose(0,1).contiguous().view(len(current), 256)
       predicted = self.output(hidden)
...
#         print(probs)
         for i in range(len(current)):
             assert "@" in current[i], current[i]
             try:
                premise, hypothesis = current[i].strip().split("@")
                print("\t".join([premise, hypothesis, str(float(probs[i,0]))]), file=outFile)
             except ValueError:
                print(current[i])
   #             assert False
       if getPredictions:
...
#         return 2*prediction.float()-1 # for binary prediction
         ...
         return loss.sum(), args.batchSize, float((prediction == target).float().mean())
...
while epochs < 50:
  ...
  try:
    while True:
       batch = [next(labeledData) for _ in range(args.batchSize)]
#       print(batch)
       counter += 1
    ...
    print("Next Epoch", counter*args.batchSize, epochs) 
...
with open('/u/scr/mhahn/PRETRAINED/GLUE/glue_data/RTE/dev_alternatives_LSTMpredictions.tsv', "w") as outFile:
 for group in ["c", "d"]:
   with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/RTE/dev_alternatives_{group}_predictions_fairseq.tsv", "r") as inFile:
     print("\t".join(['Sentence', 'Predicted']), file=outFile)
     for line in inFile:
       if len(line) < 3:
           continue
       premise, hypothesis, __, _ = line.strip().split("\t")
       sentence = premise + " @ " + hypothesis

       counter += 1
       printHere = (counter % 100 == 0)
       loss, wordNum, accuracy = model.forward([(None, (sentence, 1))], printHere, train=False, outFile=outFile)
       decay = 0.9999 ** wordNum
       overallIndices += 1
       if printHere:
           print(" ".join(list(map(str,[__file__, args.language, myID, counter, epochs ]))))
