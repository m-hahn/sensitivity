...
with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/{args.dataset}/processed/{devName}.raw.input0", "r") as inFile0:
   data_dev0 = inFile0.read().strip().split("\n")
if args.dataset not in ["CoLA", "SST-2"]:
  with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/{args.dataset}/processed/{devName}.raw.input1", "r") as inFile1:
     data_dev1 = inFile1.read().strip().split("\n")
with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/{args.dataset}/processed/{devName}.label", "r") as inFile0:
   label_dev = [x for x in inFile0.read().strip().split("\n")]


with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/{args.dataset}/processed/train.raw.input0", "r") as inFile0:
   data_train0 = inFile0.read().strip().split("\n")
if args.dataset not in ["CoLA", "SST-2"]:
  with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/{args.dataset}/processed/train.raw.input1", "r") as inFile1:
     data_train1 = inFile1.read().strip().split("\n")
with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/{args.dataset}/processed/train.label", "r") as inFile0:
...
if args.dataset in ["CoLA", "SST-2"]:
  ...
  data_train = [(clean_str(x).split(" "), clean_str(y).split(" ")) for x, y in zip(data_train0, data_train1)]
  data_dev = [(clean_str(x).split(" "), clean_str(y).split(" ")) for x, y in zip(data_dev0, data_dev1)]
...
class Model(nn.Module):
   def __init__(self):
       ...
       self.hidden = torch.nn.Linear(1024, 256)
   ...
   def forward(self, current, printHere=False, computeAccuracy=False, train=True, getPredictions=False, outFile=None):
       ...
       premises = [x[0] for x in current]
       hypotheses = [x[1] for x in current]
       lengths_premises = list(map(len, premises))
       lengths_hypotheses = list(map(len, hypotheses))
       maxLength_premises = max(lengths_premises)
       maxLength_hypotheses = max(lengths_hypotheses)
       input_words_premises = [[encodeWord(x) for x in y] + [2 for _ in range(maxLength_premises-len(y))] for y in premises]
       input_words_hypotheses = [[encodeWord(x) for x in y] + [2 for _ in range(maxLength_hypotheses-len(y))] for y in hypotheses]
       ...
       words_layer_premise = self.words_embeddings(Variable(torch.LongTensor(input_words_premises).to(device)))
       words_layer_hypothesis = self.words_embeddings(Variable(torch.LongTensor(input_words_hypotheses).to(device)))
       if train:
         words_layer_premise = self.dropout(words_layer_premise)
         words_layer_hypothesis = self.dropout(words_layer_hypothesis)
       representation_premise, _ = self.rnn1(words_layer_premise.transpose(0,1))
       representation_hypothesis, _ = self.rnn2(words_layer_hypothesis.transpose(0,1))
       #print(representation_premise.size())
       representation_premise = representation_premise.mean(dim=0)
       representation_hypothesis = representation_hypothesis.mean(dim=0)
       convolved = torch.cat([representation_premise, representation_hypothesis, representation_premise-representation_hypothesis, representation_premise*representation_hypothesis], dim=1)
       ...
       if outFile is not None:
          for i in range(len(current)):
             print("\t".join([" ".join(premises[i]), " ".join(hypotheses[i]), str(int(target[i])), str(int(prediction[i]))]), file=outFile)
...
#       print(batch)
  ...
  accuracyMeanDev = 0
  overallIndices = 0
  with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/MNLI/dev-labels-{__file__}.tsv", "w") as outFile:
   print("\t".join(['Sentence', 'Target', 'Predicted']), file=outFile)
   try:
    while True:
       batch = []
       try:
         for _ in range(batchSize):
           batch.append(next(labeledData))
       except StopIteration:
          if len(batch) == 0:
              break
       counter += 1
