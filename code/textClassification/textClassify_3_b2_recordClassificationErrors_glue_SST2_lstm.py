   ...
   def forward(self, current, printHere=False, computeAccuracy=False, train=True, getPredictions=False, outFile=None):
       ...
       if outFile is not None:
          for i in range(len(current)):
             print("\t".join([" ".join(current[i]), str(int(target[i])), str(int(prediction[i]))]), file=outFile)
       if getPredictions:
...
#       print(batch)
  ...
  accuracyMeanDev = 0
  overallIndices = 0
  with open("/u/scr/mhahn/PRETRAINED/GLUE/glue_data/SST-2/dev-labels-LSTM.tsv", "w") as outFile:
