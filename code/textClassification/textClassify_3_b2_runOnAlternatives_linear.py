...
#         quit()
         return predicted[:, 1] # for LINEAR prediction
...
predictions_total_variance = predictions_total.pow(2).mean() - predictions_total.mean().pow(2)
...
#       valuesPerVariant[variant] = 1 if alternatives_predictions_binary[variant] == "1" else -1
     #   print(variant)
        valuesPerVariant[variant] = float(predictionsBySentence[variant])
   ...
   for subset in variants_dict:
       ...
       varianceBySubset[subset] = variance(values) / predictions_total_variance
   #    print(values, varianceBySubset[subset])
