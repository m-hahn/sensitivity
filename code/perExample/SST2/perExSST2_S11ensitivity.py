...
with open("/u/scr/mhahn/sensitivity/sensitivities/s11ensitivities_estimateS11ensitivity_SST2.py", "r") as inFile:
  s11ensitivities = [x.split("\t") for x in inFile.read().strip().split("\n")]
  header_s11ensitivities = s11ensitivities[0]
  s11ensitivities = dict([(x[0].strip(), x) for x in s11ensitivities[1:]])
...
with open(f"../outputs/{__file__}.tsv", "w") as outFile:
 print("\t".join(["Sentence", "Length", "Label", "BOWPrediction", "FloatSensitivity", "FloatS11ensitivity", "SubspansPos", "SubspansNeut", "SubspansNeg", "RobertaPrediction"]), file=outFile)
 for x in sensitivities:
  if x not in bowPerformance:
    print("MISSING", x)
    continue
#  print(bowPerformance[x], sensitivities[x])
  print("\t".join([str(y) for y in [x, len(x.split(" ")), bowPerformance[x][1], bowPerformance[x][2], sensitivities[x][1], s11ensitivities[x][1], features[x]["subspans_positive"], features[x]["subspans_neutral"], features[x]["subspans_negative"], roberta[x][3]]]), file=outFile)
