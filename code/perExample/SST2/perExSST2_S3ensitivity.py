...
with open(f"../outputs/{__file__}.tsv", "w") as outFile:
 print("\t".join(["Sentence", "Length", "Label", "BOWPrediction", "FloatSensitivity", "FloatS3ensitivity", "SubspansPos", "SubspansNeut", "SubspansNeg", "RobertaPrediction"]), file=outFile)
 for x in sensitivities:
  if x not in bowPerformance:
    print("MISSING", x)
    continue
#  print(bowPerformance[x], sensitivities[x])
  print("\t".join([str(y) for y in [x, len(x.split(" ")), bowPerformance[x][1], bowPerformance[x][2], sensitivities[x][1], s3ensitivities[x][1], features[x]["subspans_positive"], features[x]["subspans_neutral"], features[x]["subspans_negative"], roberta[x][3]]]), file=outFile)
