...
with open(f"/u/scr/mhahn/PRETRAINED/parsing/en_ewt-ud-dev_alternatives_c_predictions.txt", "r") as inFile:
  for line in inFile:
     ...
     if word[0] != "▁":
       continue
     alternatives_predictions_categorical[sentence.strip()] = relation
     alternatives_predictions_continuous[sentence.strip()] = relation_vector
...
#         assert False, sentence
  ...
  # print((result))
   ...
   #  print(variant)
     try:
#       valuesPerVariant[variant] = alternatives_predictions_categorical[variant]
       valuesPerVariant[variant] = alternatives_predictions_continuous[variant]
   ...
   for subset in variants_dict:
       values = torch.FloatTensor([ [float(y) for y in valuesPerVariant[x].split(" ")] for x in variants_dict[subset]])
  #     print(values)
 #      print(originalPrediction)
#       quit()
       #print(subset, mean(values), variance(values))
       varianceBySubset[subset] = 4*float((values.mean(dim=0) - originalPrediction).pow(2).max())
