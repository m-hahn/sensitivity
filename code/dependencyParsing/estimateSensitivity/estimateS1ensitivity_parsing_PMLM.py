...
print(str(alternatives_predictions_continuous)[:5000])
#quit()
...
#         print(sentenceToWord[var]) # this is for sanity-checking
   ...
   for subset in variants_dict:
       values = torch.FloatTensor([ [float(y) for y in alternatives_predictions_continuous[x.strip()].split(" ")] for x in variants_dict[subset]])
       varianceBySubset[subset] = 4*float((values.mean(dim=0) - values).pow(2).mean(dim=0).max())
       if str(varianceBySubset[subset]) == "nan":
          del varianceBySubset[subset]
       else:
          assert varianceBySubset[subset] <= 1.1, (varianceBySubset[subset], values)
