   ...
   for partition in ["train", "dev"]:
     for sentence in CorpusIterator(args.language,partition).iterator():
      for line in sentence:
          ...
          keys.add(line["head"]-line["index"] if line["head"] != 0 else -1000)
 ...
 #      self.rnn2 = nn.LSTM(300, 128, 1, bidirectional=True)
   ...
   def forward(self, current, printHere=False, computeAccuracy=False, train=True, getPredictions=False):
       ...
       if getPredictions:
          ...
          print(current[0])
#       print(current[0])
       targets = [stoi_labels[x[0]["head"]-x[0]["index"] if x[0]["head"] != 0 else -1000] for x in current]
       surrounds = [[y["word"] for y in x[1]] for x in current]
       centers = [1 for x in current]
       lengths_surrounds = list(map(len, surrounds))
       maxLength_surrounds = max(lengths_surrounds)
       input_words_surrounds = [[1] + [2] for y in surrounds]
       self.optimizer.zero_grad()

       predicted = self.output(torch.zeros( len(current), 256))
...
for z in partitionsDev:
  for y in z:
    counts[y[0]["head"]-y[0]["index"] if y[0]["head"] != 0 else -1000] += 1
    sumCounts += 1
