...
itos_labels = itos_deps
stoi_labels = stoi_deps
 ...
 #      self.rnn2 = nn.LSTM(300, 128, 1, bidirectional=True)
   ...
   def forward(self, current, printHere=False, computeAccuracy=False, train=True, getPredictions=False):
       ...
       if getPredictions:
          ...
          print(current[0])
#       print(current[0])
       targets = [stoi_labels[x[0]["dep"]] for x in current]
       ...
       words_layer_premise = torch.cat([words_layer_surrounds, words_layer_centers.unsqueeze(1)], dim=1)
       if train:
