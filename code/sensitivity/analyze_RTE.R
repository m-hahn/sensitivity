...
lstm = read.csv("/u/scr/mhahn/sensitivity/sensitivities/s1ensitivities_estimateS1ensitivity_RTE_cd_LSTM_getSensitivityParts.py", sep="\t", quote="#") %>% rename(LSTM_Sensitivity = BinaryS1ensitivity)
roberta = read.csv("/u/scr/mhahn/sensitivity/sensitivities/s1ensitivities_estimateS1ensitivity_RTE_cd.py", sep="\t", quote="#")
...
ggsave(plot, file="roberta-lstm-rte.pdf", height=3, width=3)

write.table(data, file="lstm-rte.tsv", sep="\t", quote=F)
