...
lstm = read.csv("/u/scr/mhahn/sensitivity/sensitivities/s1ensitivities_estimateS1ensitivity_lstm_QQP.py", sep="\t", quote="#") %>% rename(LSTM_Sensitivity = BinaryS1ensitivity)
roberta = read.csv("/u/scr/mhahn/sensitivity/sensitivities/s1ensitivities_estimateS1ensitivity_QQP.py", sep="\t", quote="#")
...
ggsave(plot, file="roberta-lstm-qqp.pdf", height=3, width=3)

write.table(data, file="lstm-qqp.tsv", sep="\t", quote=F)
